...
- **Decisions & Notes**:
  ...
  - Phase 5: 131/131 tests pass.
  - Phase 6: 139/139 tests pass across unit, security, and integration suites (`test_tools_vision.py`).
  ...
  - Visual observations verified: observations require non-empty `limitation` field, reject authoritative dimensional measurement claims, and support one-shot JSON repair.
  - Static egress scanner (`scripts/scan_egress.py`) confirmed CLEAN with 0 findings across 53 scanned files.
